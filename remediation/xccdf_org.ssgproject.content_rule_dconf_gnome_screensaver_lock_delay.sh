...
echo "[*] Applying remediation for: $RULE_ID (configure GNOME screensaver lock delay)"
...
# -------------------------
# Handle lock for lock-delay
# -------------------------

# Comment out existing lock-delay locks in other databases (excluding distro, ibus, local.d)
mapfile -t LOCKFILES < <(grep -R "^/org/gnome/desktop/screensaver/lock-delay$" "$DCONF_DB_DIR" 2>/dev/null \
    | grep -v 'distro\|ibus\|local.d' | cut -d':' -f1 | sort -u)

if [ "${#LOCKFILES[@]}" -ne 0 ]; then
    sed -i -E "s|^/org/gnome/desktop/screensaver/lock-delay$|#&|" "${LOCKFILES[@]}" || true
fi

LOCKFILE="$LOCKS_DIR/00-security-settings-lock"
touch "$LOCKFILE"

if ! grep -q "^/org/gnome/desktop/screensaver/lock-delay$" "$LOCKFILE"; then
    echo "/org/gnome/desktop/screensaver/lock-delay" >> "$LOCKFILE"
fi
...
# -------------------------
# Handle lock-delay value
# -------------------------

LOCK_DELAY_VALUE='5'
DCONFFILE="$LOCAL_DIR/00-security-settings"

# Comment out existing lock-delay settings in other databases (excluding distro, ibus, local.d)
mapfile -t SETTINGSFILES < <(grep -R "\[org/gnome/desktop/screensaver\]" "$DCONF_DB_DIR" 2>/dev/null \
    | grep -v 'distro\|ibus\|local.d' | cut -d':' -f1 | sort -u)

if [ "${#SETTINGSFILES[@]}" -ne 0 ]; then
    if grep -q "^\s*lock-delay\s*=" "${SETTINGSFILES[@]}" 2>/dev/null; then
        sed -Ei "s/(^\s*)lock-delay(\s*=)/#\1lock-delay\2/g" "${SETTINGSFILES[@]}" || true
    fi
fi
...
# Set lock-delay=uint32 5
escaped_value="$(printf '%s' "uint32 ${LOCK_DELAY_VALUE}" | sed -e 's/\\/\\\\/g')"
if grep -q "^\s*lock-delay\s*=" "$DCONFFILE"; then
    sed -i "s/^\s*lock-delay\s*=.*/lock-delay=${escaped_value}/" "$DCONFFILE"
else
...
echo "[+] Remediation complete: GNOME screensaver lock delay set to uint32 5 and locked via dconf (local.d)."
