...
echo "[*] Applying remediation for: $RULE_ID (Set strong SSHD KexAlgorithms)"
...
echo "    -> Configuring KexAlgorithms to: $sshd_strong_kex"
...
# 1. Remove existing KexAlgorithms directives from main config and all drop-in files
LC_ALL=C sed -i "/^\s*KexAlgorithms\s\+/Id" "/etc/ssh/sshd_config" || true
LC_ALL=C sed -i "/^\s*KexAlgorithms\s\+/Id" "/etc/ssh/sshd_config.d"/*.conf 2>/dev/null || true

# 2. Ensure TARGET_FILE is clean and ends with a newline
LC_ALL=C sed -i "/^\s*KexAlgorithms\s\+/Id" "$TARGET_FILE" || true
sed -i -e '$a\' "$TARGET_FILE" || true # ensure file has newline at the end

# 3. Insert the strong KexAlgorithms setting at the beginning of the file
cp "$TARGET_FILE" "$TARGET_FILE.bak"
printf '%s\n' "KexAlgorithms $sshd_strong_kex" > "$TARGET_FILE"
cat "$TARGET_FILE.bak" >> "$TARGET_FILE"
rm "$TARGET_FILE.bak"

echo "[+] Remediation complete. KexAlgorithms configured. SSH service may need restart."
