...
echo "[*] Applying remediation for: $RULE_ID (Enable Reverse Path Filtering on all interfaces)"
...
# Comment out any occurrences of net.ipv4.conf.all.rp_filter from relevant config files
...
#
echo "[*] Setting runtime value to $sysctl_net_ipv4_conf_all_rp_filter_value"
if ! /bin/false ; then
...
echo "[+] Remediation complete: $SYSCTL_KEY is set to $sysctl_net_ipv4_conf_all_rp_filter_value (runtime and persistent)."
