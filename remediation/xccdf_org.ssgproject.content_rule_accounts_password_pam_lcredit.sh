...
formatted_output="lcredit = ${var_password_pam_lcredit}"

# If the key exists, replace it; otherwise append
if LC_ALL=C grep -qi -m 1 '^[[:space:]]*lcredit[[:space:]]*=' "$PWQ_CONF"; then
    escaped_formatted_output=$(printf '%s\n' "$formatted_output" | sed -e 's|/|\\/|g')
    LC_ALL=C sed -i --follow-symlinks "s/^[[:space:]]*lcredit[[:space:]]*=.*/${escaped_formatted_output}/I" "$PWQ_CONF"
else
