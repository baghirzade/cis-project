...
echo "[*] Applying remediation for: $RULE_ID (ensure /etc/issue.net is group-owned by GID 0)"
...
chgrp --no-dereference "$TARGET" "$TARGET" 2>/dev/null || chgrp --no-dereference "$newgroup" "$TARGET"
