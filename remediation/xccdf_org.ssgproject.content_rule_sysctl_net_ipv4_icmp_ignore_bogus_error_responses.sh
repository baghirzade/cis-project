...
echo "[*] Applying remediation for: $RULE_ID (Ignore bogus IPv4 ICMP error responses)"
...
SYSCTL_KEY='net.ipv4.icmp_ignore_bogus_error_responses'

echo "[*] Commenting out existing '${SYSCTL_KEY}' occurrences in config files..."

# Comment out any occurrences of net.ipv4.icmp_ignore_bogus_error_responses from relevant config files
...
#
echo "[*] Setting runtime value to $sysctl_net_ipv4_icmp_ignore_bogus_error_responses_value"
if ! /bin/false ; then
...
echo "[+] Remediation complete: $SYSCTL_KEY is set to $sysctl_net_ipv4_icmp_ignore_bogus_error_responses_value (runtime and persistent)."
