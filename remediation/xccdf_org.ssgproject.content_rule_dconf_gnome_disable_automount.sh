...
echo "[*] Applying remediation for: $RULE_ID (disable GNOME media automount via dconf)"
...
# Configure gdm profile: user-db:user + system-db:gdm (kept for consistency with upstream)
: > "$GDM_PROFILE"
...
# Comment out existing automount settings in other databases (excluding distro, ibus, local.d)
...
if [ "${#SETTINGSFILES[@]}" -ne 0 ]; then
    if grep -q "^\s*automount\s*=" "${SETTINGSFILES[@]}" 2>/dev/null; then
        sed -Ei "s/(^\s*)automount(\s*=)/#\1automount\2/g" "${SETTINGSFILES[@]}" || true
    fi
fi
...
# Set automount=false in local.d
escaped_value="$(printf '%s' "false" | sed -e 's/\\/\\\\/g')"
if grep -q "^\s*automount\s*=" "$DCONFFILE"; then
    sed -i "s/^\s*automount\s*=.*/automount=${escaped_value}/" "$DCONFFILE"
else
...
mapfile -t LOCKFILES < <(grep -R "^/org/gnome/desktop/media-handling/automount$" "$DCONF_DB_DIR" 2>/dev/null \
    | grep -v 'distro\|ibus\|local.d' | cut -d':' -f1 | sort -u)

if [ "${#LOCKFILES[@]}" -ne 0 ]; then
    sed -i -E "s|^/org/gnome/desktop/media-handling/automount$|#&|" "${LOCKFILES[@]}" || true
fi

LOCKFILE="$LOCKS_DIR/00-security-settings-lock"
touch "$LOCKFILE"

if ! grep -q "^/org/gnome/desktop/media-handling/automount$" "$LOCKFILE"; then
    echo "/org/gnome/desktop/media-handling/automount" >> "$LOCKFILE"
fi

chmod -R u=rwX,go=rX "$DCONF_DB_DIR"
(umask 0022 && dconf update || true)

echo "[+] Remediation complete: GNOME media automount disabled and locked via dconf (local.d)."
