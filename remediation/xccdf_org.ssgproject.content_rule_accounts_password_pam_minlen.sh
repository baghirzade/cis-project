...
formatted_output="minlen = ${var_password_pam_minlen}"

# If the key exists, replace it; otherwise append
if LC_ALL=C grep -qi -m 1 '^[[:space:]]*minlen[[:space:]]*=' "$PWQ_CONF"; then
    escaped_formatted_output=$(printf '%s\n' "$formatted_output" | sed -e 's|/|\\/|g')
    LC_ALL=C sed -i --follow-symlinks "s/^[[:space:]]*minlen[[:space:]]*=.*/${escaped_formatted_output}/I" "$PWQ_CONF"
else
