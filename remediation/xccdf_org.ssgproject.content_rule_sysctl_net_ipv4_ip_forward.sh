...
echo "[*] Applying remediation for: $RULE_ID (Disable IPv4 packet forwarding)"
...
# Comment out any occurrences of net.ipv4.ip_forward from relevant config files
...
sysctl_net_ipv4_ip_forward_value='0'


#
# Set runtime for net.ipv4.ip_forward
#
echo "[*] Setting runtime value to $sysctl_net_ipv4_ip_forward_value"
if ! /bin/false ; then
...
printf -v formatted_output "%s = %s" "$stripped_key" "$sysctl_net_ipv4_ip_forward_value"
...
echo "[+] Remediation complete: $SYSCTL_KEY is set to $sysctl_net_ipv4_ip_forward_value (runtime and persistent)."
