...
echo "[*] Applying remediation for: $RULE_ID (Enable logging of Martian packets by default)"
...
SYSCTL_KEY='net.ipv4.conf.default.log_martians'

echo "[*] Commenting out existing '${SYSCTL_KEY}' occurrences in config files..."

# Comment out any occurrences of net.ipv4.conf.default.log_martians from relevant config files
...
#
echo "[*] Setting runtime value to $sysctl_net_ipv4_conf_default_log_martians_value"
if ! /bin/false ; then
...
echo "[+] Remediation complete: $SYSCTL_KEY is set to $sysctl_net_ipv4_conf_default_log_martians_value (runtime and persistent)."
