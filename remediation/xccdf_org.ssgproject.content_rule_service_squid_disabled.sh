...
echo "[*] Applying remediation for: $RULE_ID (Disable Squid service)"

# Remediation is applicable only if 'squid' package is installed and linux-base is present
...
echo "[+] Remediation complete. Squid service is disabled and masked."

else
    >&2 echo 'Remediation is not applicable, squid package is not installed or platform check failed.'
fi
