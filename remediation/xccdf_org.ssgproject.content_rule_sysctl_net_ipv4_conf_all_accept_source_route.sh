...
echo "[*] Applying remediation for: $RULE_ID (Disable IPv4 source routing on all interfaces)"
...
# Comment out any occurrences of net.ipv4.conf.all.accept_source_route from relevant config files
...
#
echo "[*] Setting runtime value to $sysctl_net_ipv4_conf_all_accept_source_route_value"
if ! /bin/false ; then
...
echo "[+] Remediation complete: $SYSCTL_KEY is set to $sysctl_net_ipv4_conf_all_accept_source_route_value (runtime and persistent)."
