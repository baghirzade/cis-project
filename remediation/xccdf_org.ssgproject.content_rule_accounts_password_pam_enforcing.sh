...
# Prepare "enforcing = 1" line
var_password_pam_enforcing='1'
formatted_output="enforcing = ${var_password_pam_enforcing}"

# If the key exists, replace it; otherwise append
if LC_ALL=C grep -qi -m 1 '^[[:space:]]*enforcing[[:space:]]*=' "$PWQ_CONF"; then
    escaped_formatted_output=$(printf '%s\n' "$formatted_output" | sed -e 's|/|\\/|g')
    LC_ALL=C sed -i --follow-symlinks "s/^[[:space:]]*enforcing[[:space:]]*=.*/${escaped_formatted_output}/I" "$PWQ_CONF"
else
