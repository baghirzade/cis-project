...
check_inactive_lock() {
  ...
  if [[ -z "$def_inactive" || "$def_inactive" -lt 0 ]]; then
    log_warn "$CONTROL_ID: Default INACTIVE not set or -1 (no lock). Expected <= 30 days."
  else
  ...
  # Informational per-user view
  local user uid inactive
  while IFS=: read -r user _ uid _; do
    [[ "$uid" -lt 1000 ]] && continue
    [[ "$user" == "nobody" ]] && continue
    inactive=$(chage -l "$user" | awk -F': ' '/Account inactive/ {print $2}')
    log_info "$CONTROL_ID: User '$user' Account inactive: $inactive"
  done < /etc/passwd
}
