...
TITLE="Ensure Rsyncd service is disabled and masked"
...
run() {
    ...
    if ! command -v "$SYSTEMCTL_EXEC" &> /dev/null; then
        echo "WARN|$RULE_ID|systemctl command not found. Cannot determine service status."
        return 1
    fi
    
    # Check if the service unit file exists
    if ! "$SYSTEMCTL_EXEC" -q list-unit-files --type=service | grep -q "$SERVICE_NAME"; then
        echo "OK|$RULE_ID|Service $SERVICE_NAME unit file not found (Implied compliance if package is not installed)."
        return 0
    fi
    ...
    if [ "$IS_MASKED" = "masked" ]; then
        ...
        echo "WARN|$RULE_ID|Service $SERVICE_NAME is not masked (Current: $IS_MASKED). Must be 'masked'."
        return 1
    fi
    ...
    if [ "$IS_ACTIVE" = "inactive" ]; then
        ...
        echo "WARN|$RULE_ID|Service $SERVICE_NAME is running or failed (Current: $IS_ACTIVE). Must be 'inactive'."
        return 1
    fi
}
