...
TITLE="Ensure /etc/gshadow- has user ownership set to root (UID 0)"

run() {
    local TARGET_FILE="/etc/gshadow-"
    local EXPECTED_UID="0"
    
    # 1. Check if the file exists
    if [ ! -f "$TARGET_FILE" ]; then
        # If the backup file doesn't exist, it's considered compliant (or not applicable)
