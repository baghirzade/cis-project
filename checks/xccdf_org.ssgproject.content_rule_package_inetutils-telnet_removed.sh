...
RULE_ID="xccdf_org.ssgproject.content_rule_package_inetutils-telnet_removed"
TITLE="Ensure inetutils-telnet package is removed"
...
run() {
    ...
    if dpkg-query --show --showformat='${db:Status-Status}' "$PACKAGE_NAME" 2>/dev/null | grep -q '^installed$'; then
        echo "WARN|$RULE_ID|Package $PACKAGE_NAME is installed. Telnet uses unencrypted communication."
        return 1
    else
