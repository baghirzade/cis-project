...
TITLE="Ensure TFTP service (tftpd-hpa) is disabled and masked"
...
run() {
    ...
    if ! "$SYSTEMCTL_EXEC" list-unit-files --type=service | grep -q "$SERVICE_NAME"; then
        echo "OK|$RULE_ID|Service $SERVICE_NAME unit file not found (Implied compliance if tftp package is not installed)."
        return 0
    fi
