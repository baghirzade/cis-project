...
RULE_ID="xccdf_org.ssgproject.content_rule_banner_etc_motd_cis"
TITLE="/etc/motd must contain the CIS-compliant message of the day banner"
...
run() {
    ...
    if [ "$file_content" = "$CIS_BANNER_TEXT" ]; then
        echo "OK|$RULE_ID|/etc/motd contains the CIS-compliant banner text"
    else
        echo "WARN|$RULE_ID|/etc/motd banner does not match the CIS-required text"
    fi
}
