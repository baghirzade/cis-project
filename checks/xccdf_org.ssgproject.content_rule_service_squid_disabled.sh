...
TITLE="Ensure Squid service is disabled and masked"

run() {
    # Check platform applicability: Check if 'squid' package is installed and if linux-base is present (Debian/Ubuntu)
    if ! ( dpkg-query --show --showformat='${db:Status-Status}' 'squid' 2>/dev/null | grep -q '^installed$' && dpkg-query --show --showformat='${db:Status-Status}' 'linux-base' 2>/dev/null | grep -q '^installed$' ); then
        echo "NOTAPPL|$RULE_ID|'squid' package is not installed or platform check failed. No action required."
        return 0
    fi
