...
RULE_ID="xccdf_org.ssgproject.content_rule_banner_etc_issue_cis"
TITLE="/etc/issue must contain the CIS-compliant login banner"
...
run() {
    ...
    # Read file content, trim trailing newline(s)
    file_content="$(sed -e ':a' -e '/^\n*$/{$d;N;ba' -e '}' "$BANNER_FILE")"

    if [ "$file_content" = "$CIS_BANNER_TEXT" ]; then
        echo "OK|$RULE_ID|/etc/issue contains the CIS-compliant banner text"
    else
        echo "WARN|$RULE_ID|/etc/issue banner does not match the CIS-required text"
    fi
}
