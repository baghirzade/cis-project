...
TITLE="Ensure Reverse Path Filtering is enabled on all interfaces"
...
run() {
    ...
    if [ "$CURRENT_RUNTIME_VALUE" != "$EXPECTED_VALUE" ]; then
        echo "WARN|$RULE_ID|Runtime value is $CURRENT_RUNTIME_VALUE, expected $EXPECTED_VALUE (Strict Reverse Path Filtering)."
        return 1
    fi
