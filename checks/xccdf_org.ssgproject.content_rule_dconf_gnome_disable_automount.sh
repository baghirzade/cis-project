...
TITLE="GNOME must have removable media automount disabled via dconf"
...
run() {
    ...
    # Check settings: [org/gnome/desktop/media-handling] + automount=false in local.d
    if ! grep -R "^\s*\[org/gnome/desktop/media-handling\]\s*$" "$LOCAL_DIR" >/dev/null 2>&1 || \
       ! grep -R "^\s*automount\s*=\s*false\s*$" "$LOCAL_DIR" >/dev/null 2>&1; then
        echo "WARN|$RULE_ID|automount=false is not configured under [org/gnome/desktop/media-handling] in $LOCAL_DIR"
        return 0
    fi

    # Check locks
    if [ ! -d "$LOCKS_DIR" ] || \
       ! grep -R "^\s*/org/gnome/desktop/media-handling/automount\s*$" "$LOCKS_DIR" >/dev/null 2>&1; then
        echo "WARN|$RULE_ID|Lock for /org/gnome/desktop/media-handling/automount is not present in $LOCKS_DIR"
        return 0
    fi

    echo "OK|$RULE_ID|automount=false is configured and locked for org/gnome/desktop/media-handling (local dconf)"
}
